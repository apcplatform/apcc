...
{
    resize(850, 550);
    setWindowTitle(tr("AppleCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a AppleCoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About AppleCoin"), this);
    aboutAction->setStatusTip(tr("Show information about AppleCoin"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for AppleCoin"));
    ...
    secondAuthAction->setStatusTip(tr("Second auth with your AppleCoin addresses"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("AppleCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("AppleCoin client"));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid AppleCoin address or malformed URI parameters."));
    }
